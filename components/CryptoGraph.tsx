...
export default function BitcoinTracker(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const endpoint = `${baseUrl}/simple/price?ids=bitcoin&vs_currencies=inr,usd&include_24hr_change=true`;
        const response = await axios.get<{ bitcoin: CryptoData }>(endpoint);
  ...
  return (
    <div className="bg-white h-max rounded-lg my-5 p-6">
      <div className="flex items-center">
        <div>
          <Image src="/btc.png" width={36} height={36} alt="Bitcoin" />
        </div>
        <div className="text-2xl font-semibold text-[#0B1426] pl-2">Bitcoin</div>
        <div className="text-sm text-[#5D667B] pl-2">BTC</div>
        <div className="bg-[#808A9D] px-3 py-2 text-white rounded-lg ml-7">Rank #1</div>
      </div>
      <div className="mt-8 flex">
        ...
        <div
          className={`flex items-center justify-center rounded-lg p-2 h-10 ml-10 ${
            cryptoStats && cryptoStats.inr_24h_change < 0 ? "bg-red-300/20" : "bg-green-300/20"
          }`}
        >
          <svg
            viewBox="0 0 100 100"
            className={`w-4 fill-current ${
              cryptoStats && cryptoStats.inr_24h_change < 0 ? "text-red-600 rotate-180" : "text-green-600"
            }`}
          >
          ...
          <span
            className={`ml-2 text-sm font-bold ${
              cryptoStats && cryptoStats.inr_24h_change < 0 ? "text-red-600" : "text-green-600"
            }`}
          >
            {(cryptoStats && `${Math.abs(cryptoStats.inr_24h_change).toFixed(2)}%`) || `2.18%`}
          </span>
        </div>
      ...
      <div className="lg:flex mb-4 justify-between">
        <div className="ls:text-lg text-sm font-semibold text-[#0B1426]">Bitcoin Price Chart (USD)</div>
